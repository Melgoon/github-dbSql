...
=> 실행 계획을 공유하지 못한다.
=> 해결책 바인드 변수
SQL에서 변경되는 부분을 별도로 전송을 하고 실행계획 세워진 이후에 바인딩 작업을 거쳐 실제 사용자가 원하는 변수 값으로 치환후 실행
...
SELECT 결과 여러건을 TABLE 타입의 변수에 저장할 수 있지만 메모리는 한정적이기 때문에 많은양의 데이터를 담기에는 제한이 따름.
...
부서 테이블을 활용하여 모든 행에 대해 부서번호와 부서 이름을 CURSOR를 통해 FETCH, FETCH 된 결과를 확인;

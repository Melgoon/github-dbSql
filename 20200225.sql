...
CONNECT BY LEVEL <= TO_CHAR(LAST_DAY(TO_DATE(p_yyyymm || '01', 'YYYYMMDD')), 'DD');
    ...
    FOR daily_row IN (SELECT * FROM cycle) LOOP
            DBMS_OUTPUT.PUT_LINE(daily_row.cid || ' ' || daily_row.pid || '' || daily_row.day || '' || daily_row.out);
...
create_daily_sales 프로시져에서 건별로 insert 하던 로직은 INSERT SELECT 구문, ONE-QURRY 형태로
변형 하여 속도를 단축;
...
emp테이블을 통해서는 부서이름을 알 수가 없다. (부서이름 dept 테이블에 존재)
==> 1. join
...
CREATE OR REPLACE FUNCTION getEmpName(p_empno emp.ename%TYPE) RETURN VARCHAR2 IS
    v_ename emp.ename%TYPE;
BEGIN
    SELECT ename INTO v_ename
    FROM emp

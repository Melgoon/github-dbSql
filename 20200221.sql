...
TABLE type 테이블 타입
점 : 스칼라 변수
선 : %ROWTYPE, recdrd type
면 : talbe type
    어떤 선(%ROWTYPE, RECORD TYPE)을 저장할 수 있는지 인덱스 타입은 무엇인지;
    
DEPT 테이블의 정보를 담을 수 있는 TABLE TYPE을 선언
기존에 배운 스칼라 타입, ROWTYPE에서는 한 행의 정보를 담을 수 있었지만 TABLE 타입 변수를 이용하면 여러 행의 정보를 담을 수 있다.

PL/SQL에서는 자바와 다르게 배열에 대한 인덱스가 정수로 고정되어 있지 않고 문자열도 가능하다.

그래서 TABLE 타입을 선언할 때는 인덱스에 대한 타입도 같이 명사
...
DECLARE
    ...
    v_dept_tab dept_tab; -- 변수(v_dept_tab) 타입(dept_tab)
    
BEGIN
    SELECT * BULK COLLECT INTO v_dept_tab
    FROM dept;
    --기존 스칼라 변수, record 타입을 실습시에는 한행만 조회되도록 WHERE절을 통해 제한하였다.

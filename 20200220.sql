1.leaf 노드(행)이 어떤 데이터인지 확인
2. LEVEL ==> 상향탐색시 그룹을 묶이 위해 필요한 값;
...
FROM dual
CONNECT BY LEVEL <= 29)a --EXISTS를 사용할 수 있게끔 쿼리작성
WHERE EXISTS ( SELECT 'X' FROM gis_dt 
WHERE gis_dt.dt BETWEEN dt AND TO_DATE(To_CHAR(dt, 'YYYYMMDD')|| '235959', 'YYYYMMDDHH24MISS')); --좌변을 가공하지 말라.

PL/SQL 블록 구조
DECLARE : 변수, 상수 선언 (생략이 가능하다.)
BEGIN : 로직 기술 [생략 불가] -조건문, 반복문
EXCEPTION : 예외처리(생략 가능)
DBMS_OUTPUT.PUT_LINE(msg) : JAVA의 시스아웃과 같은 역할을 한다.
END;
/
...
printdetp_p 인자로 부서번호를 받아서 해당 부서번호에 해당하는 부서이름과 지역정보를 콘솔에 출력하는 프로시져;
